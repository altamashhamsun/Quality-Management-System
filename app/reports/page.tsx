...
export default function ReportsPage() {
  ...
  async function handleSave() {
    ...
    const summary = `Major NCs: ${majorNcs.length}, Minor NCs: ${minorNcs.length}, Departments: ${planDepartments.map((d) => d.name).join(", ") || "All"}, Criteria: ${criteria.join(", ") || "—"}`;
    const content = {
  ...
  async function handlePdf() {
    ...
    try {
      ...
      const data: AuditReportPdfData = {
        ...
        dateRange: formatDateRange(selectedPlan.start_date!, selectedPlan.end_date!),
        auditors: narrative.auditors.trim() || "—",
        ...
        <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-500">
              Select an audit plan to generate a professional report from the
              NCRs raised during its audit dates.
            </p>
          </div>
          {selectedPlan && (
            <button
            ...
            {!selectedPlan ? (
              ...
              <div className="flex flex-col gap-6">
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                  <StatCard label="NCRs in scope" value={scopedRecords.length} />
                ...
                <ReportSection title="Criteria / Standards">
                  {criteria.length === 0 ? (
                    <p className="text-sm text-zinc-500">
                      No standards found. NCRs against these standards will
                      appear here when raised in the audit period.
                    </p>
                  ) : (
                ...
                <ReportSection title="Non-Conformances">
                  <NcList title="Major NC" tone="red" ncs={majorNcs} empty="No major non-conformances in this period." />
                  <NcList title="Minor NC" tone="yellow" ncs={minorNcs} empty="No minor non-conformances in this period." />
                </ReportSection>
                ...
                <ReportSection title="Formal Sign-Off & Evidence">
                  ...
                  <div className="mt-4">
                    <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-zinc-400">
                      Appendices / Evidence (photos from audit dates)
                    </p>
                    {scopedRecords.every((r) => (r.pictures ?? []).length === 0) ? (
                      <p className="text-sm text-zinc-500">
                        No photos attached to NCRs in this period.
                      </p>
                    ) : (
